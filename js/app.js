...
function onProductsLoaded() {
  ...
  if (path.startsWith('/vacuum/') || path.startsWith('/product/')) {
    ...
    if (hasSsr && document.getElementById('reviews-archive-container')) {
      bindReviewsArchiveEvents();
    } else {
      renderReviewsArchivePage();
    }
  } else if (path.startsWith('/reviews/')) {
    const reviewSlug = path.replace('/reviews/', '').replace(/\/$/, '');
    const hasSsr = els.dedicatedArticleView &&
      !els.dedicatedArticleView.querySelector('[data-skeleton]') &&
      els.dedicatedArticleView.children.length > 0 &&
      els.dedicatedArticleView.innerHTML.trim().length > 200;

    if (hasSsr && document.getElementById('review-article-detail')) {
      bindReviewDetailPageEvents();
    } else {
      renderReviewDetailPage(reviewSlug);
    }
...
function handleRouteFromUrl() {
  ...
  // Reviews Section Hub: /reviews or /reviews/
    ...
    showArticleView();
    updateBreadcrumbs('Reviews', 'Vacuum Cleaner Reviews');
    document.title = 'Vacuum Cleaner Reviews | VacCompare';
    updateMetaDescription('Read in-depth vacuum cleaner reviews, hands-on testing insights, performance analysis and expert recommendations from VacCompare.');

    if (hasSsrContent && document.getElementById('reviews-archive-container')) {
      bindReviewsArchiveEvents();
    } else {
      renderReviewsArchivePage();
    }
  ...
  // Individual Review Article: /reviews/:slug
    ...
    if (hasSsrContent && document.getElementById('review-article-detail')) {
      bindReviewDetailPageEvents();
    } else {
      renderReviewDetailPage(reviewSlug);
    }
...
/* ---------------------------------------------------------------- */
/* Reviews Section Client-Side Architecture                         */
/* ---------------------------------------------------------------- */

let cachedReviewArticlesList = null;

async function fetchReviewArticlesData() {
  if (cachedReviewArticlesList && cachedReviewArticlesList.length > 0) {
    return cachedReviewArticlesList;
  }
  try {
    const res = await fetch('/api/review-articles');
    if (res.ok) {
      cachedReviewArticlesList = await res.json();
      return cachedReviewArticlesList;
    }
  } catch (err) {
    console.warn('Could not fetch review articles from API:', err);
  }
  return [];
}

function renderClientReviewStars(rating) {
  const full = Math.floor(rating || 4.5);
  const half = (rating || 4.5) - full >= 0.4;
  let html = '';
  for (let i = 0; i < full; i++) html += '<i class="fa-solid fa-star"></i>';
  if (half) html += '<i class="fa-solid fa-star-half-stroke"></i>';
  const totalStars = full + (half ? 1 : 0);
...
function renderClientArticleContentHtml(content) {
  if (!content) return '';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('### ')) {
      if (inList) { html += '</ul>\n'; inList = false; }
      html += `<h3 class="text-lg sm:text-xl font-black text-slate-900 mt-6 mb-3 tracking-tight">${escapeHtml(line.slice(4))}</h3>\n`;
    } else if (line.startsWith('## ')) {
      if (inList) { html += '</ul>\n'; inList = false; }
      html += `<h2 class="text-xl sm:text-2xl font-black text-slate-900 mt-8 mb-4 tracking-tight border-b border-slate-100 pb-2">${escapeHtml(line.slice(3))}</h2>\n`;
    } else if (line.startsWith('# ')) {
      if (inList) { html += '</ul>\n'; inList = false; }
      html += `<h1 class="text-2xl sm:text-3xl font-black text-slate-900 mt-10 mb-4 tracking-tight">${escapeHtml(line.slice(2))}</h1>\n`;
    } else if (line.startsWith('- ') || line.startsWith('* ')) {
      if (!inList) {
        html += '<ul class="space-y-2 my-4 list-disc list-inside text-slate-700 text-sm sm:text-base leading-relaxed pl-2">\n';
        inList = true;
      }
      html += `  <li>${escapeHtml(line.slice(2))}</li>\n`;
    } else if (line.startsWith('> ')) {
      if (inList) { html += '</ul>\n'; inList = false; }
      html += `<blockquote class="p-4 my-4 bg-slate-50 border-l-4 border-brand-500 rounded-r-xl text-slate-700 italic text-sm sm:text-base">${escapeHtml(line.slice(2))}</blockquote>\n`;
    } else {
      if (inList) { html += '</ul>\n'; inList = false; }
      html += `<p class="text-sm sm:text-base text-slate-700 leading-relaxed my-3 font-normal">${escapeHtml(line)}</p>\n`;
    }
  }
...
function renderClientReviewCard(a) {
...
async function renderReviewsArchivePage() {
  if (!els.dedicatedArticleView) return;

  // Show clean skeleton while loading
  els.dedicatedArticleView.innerHTML = `
    <div class="space-y-8 animate-pulse text-slate-800" data-skeleton="true">
      <div class="p-10 bg-slate-900 rounded-3xl space-y-4">
        <div class="h-6 w-48 bg-slate-800 rounded"></div>
        <div class="h-10 w-96 bg-slate-800 rounded"></div>
        <div class="h-4 w-72 bg-slate-800 rounded"></div>
      </div>
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div class="lg:col-span-8 space-y-6">
          <div class="h-64 bg-white rounded-2xl border border-slate-200"></div>
          <div class="h-64 bg-white rounded-2xl border border-slate-200"></div>
        </div>
        <div class="lg:col-span-4 space-y-6">
          <div class="h-48 bg-white rounded-2xl border border-slate-200"></div>
          <div class="h-64 bg-white rounded-2xl border border-slate-200"></div>
        </div>
      </div>
    </div>
  ...
  const allArticles = await fetchReviewArticlesData();
  ...
  const cardsHtml = standardArticles.map(a => renderClientReviewCard(a)).join('\n');
    ...
    els.dedicatedArticleView.innerHTML = `
      <div id="reviews-archive-container" class="space-y-8 animate-fade-in max-w-5xl mx-auto">
        ...
        <div class="text-center py-20 bg-white rounded-3xl border border-slate-200 p-8 space-y-5 shadow-2xs">
          ...
          <button type="button" class="btn-open-review-editor inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-extrabold text-xs shadow-md transition cursor-pointer">
...
function bindReviewsArchiveEvents() {
  const searchInput = document.getElementById('review-search-input');
  const sortSelect = document.getElementById('review-sort-select');
  const catButtons = document.querySelectorAll('.review-cat-btn');
  const sidebarCatButtons = document.querySelectorAll('.review-sidebar-cat-btn');
  const sidebarBrandButtons = document.querySelectorAll('.review-sidebar-brand-btn');
  const cardsList = document.getElementById('review-cards-list');
  ...
  const resetBtn = document.getElementById('review-reset-filter-btn');

  // Open Writer Modal triggers
  document.querySelectorAll('.btn-open-review-editor').forEach(btn => {
    btn.addEventListener('click', () => openReviewArticleEditor(null));
  });
  ...
  document.querySelectorAll('.edit-review-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.preventDefault();
      e.stopPropagation();
      const articleId = btn.getAttribute('data-edit-review');
      const articles = await fetchReviewArticlesData(true);
      const article = articles.find(a => (a.id === articleId || a.slug === articleId));
      if (article) {
        openReviewArticleEditor(article);
      }
    });
  });
  ...
  function applyFiltering() {
    const cardElements = Array.from(cardsList.querySelectorAll('.review-article-card'));
    let visibleCount = 0;
    ...
    cardElements.forEach(card => {
      ...
      const matchesSearch = !searchQuery ||
        ...
        cardModel.includes(searchQuery) ||
    ...
    cardElements.forEach(card => cardsList.appendChild(card));

    if (countIndicator) {
      countIndicator.textContent = `Showing ${visibleCount} Published Review${visibleCount === 1 ? '' : 's'}`;
    }
  ...
  function setActiveCategory(catSlug) {
    currentCat = catSlug;
    catButtons.forEach(btn => {
      const slug = btn.getAttribute('data-category');
      if (slug === catSlug) {
        btn.className = 'review-cat-btn whitespace-nowrap px-4 py-2 rounded-xl text-xs font-bold transition bg-brand-600 text-white shadow-xs cursor-pointer';
      } else {
        btn.className = 'review-cat-btn whitespace-nowrap px-4 py-2 rounded-xl text-xs font-bold transition bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 cursor-pointer';
      }
    });
    applyFiltering();
  }
  ...
  sidebarCatButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      const slug = btn.getAttribute('data-filter-cat');
      setActiveCategory(slug);
      window.scrollTo({ top: 300, behavior: 'smooth' });
    });
  });

  sidebarBrandButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      const brand = btn.getAttribute('data-filter-brand');
...
async function renderReviewDetailPage(reviewSlug) {
  if (!els.dedicatedArticleView) return;

  // Show skeleton
  els.dedicatedArticleView.innerHTML = `
    <div class="max-w-4xl mx-auto space-y-8 animate-pulse text-slate-800" data-skeleton="true">
  ...
  // Always force fetch from API to get latest editorial updates
  const articles = await fetchReviewArticlesData(true);
  const matched = articles.find(a => a.slug === reviewSlug);

  if (!matched || (!matched.published && !window.location.search.includes('preview=true'))) {
    render404Page(`/reviews/${reviewSlug}`);
    updateBreadcrumbs('Error', '404 Review Not Found');
    document.title = 'Review Not Found | VacCompare';
    return;
  }

  updateBreadcrumbs('Reviews', matched.title);
  document.title = `${matched.title} | VacCompare`;
  updateMetaDescription(matched.excerpt || 'In-depth hands-on vacuum cleaner review article and performance analysis.');

  const otherReviews = articles.filter(o => o.slug !== matched.slug && o.published === true).slice(0, 3);
  const pubDate = matched.publishedDate || matched.publishDate || 'Recent';
  const imgUrl = matched.featuredImage || matched.coverImage || 'https://images.unsplash.com/photo-1558317374-067fb5f30001?auto=format&fit=crop&w=800&q=80';

  // Render authentic article content verbatim without rewriting or modifying paragraphs
  const rawContent = matched.content || (matched.sections ? matched.sections.map(s => `## ${s.heading}\n\n${s.body}`).join('\n\n') : matched.excerpt);
  const articleContentHtml = renderClientArticleContentHtml(rawContent);
    ...
    <article id="review-article-detail" class="max-w-4xl mx-auto space-y-10 animate-fade-in">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="flex flex-wrap items-center gap-2">
            ...
            <span class="px-3 py-1 rounded-xl bg-brand-50 text-brand-700 border border-brand-200 text-xs font-bold">
              ${escapeHtml(matched.category || 'Vacuum Review')}
            </span>
            ${matched.reviewType ? `<span class="px-3 py-1 rounded-xl bg-slate-100 text-slate-700 text-xs font-semibold">${escapeHtml(matched.reviewType)}</span>` : ''}
            ${!matched.published ? `<span class="px-2.5 py-1 rounded-xl bg-amber-100 text-amber-800 text-xs font-black uppercase">Draft Preview</span>` : ''}
          </div>

          <button type="button" data-edit-review="${escapeHtml(matched.id || matched.slug)}" class="edit-review-btn inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border border-slate-300 hover:bg-slate-100 text-slate-700 font-bold text-xs transition cursor-pointer">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 py-4 border-y border-slate-200 text-xs sm:text-sm text-slate-600">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-brand-600 text-white flex items-center justify-center font-bold text-base shadow-xs">
              ${escapeHtml((matched.author || 'V').charAt(0))}
            </div>
            <div>
              <div class="font-extrabold text-slate-900">${escapeHtml(matched.author || 'Editorial Team')}</div>
              <div class="text-xs text-slate-500">${escapeHtml(matched.authorRole || 'Testing Specialist')}</div>
            </div>
          </div>
          <div class="flex items-center gap-3 text-xs text-slate-500">
            <span>Published: <strong>${pubDate}</strong></span>
            ${matched.updatedDate ? `<span>• Updated: <strong>${matched.updatedDate}</strong></span>` : ''}
          </div>
        </div>
      </header>
      ...
      <section class="space-y-4">
        <div class="rounded-3xl overflow-hidden border border-slate-200 shadow-md">
          <img src="${imgUrl}" alt="${escapeHtml(matched.title)}" class="w-full h-80 sm:h-96 md:h-[450px] object-cover" />
        </div>
        ${matched.productModel ? `
          <div class="text-xs text-slate-500 italic text-center">
            Testing subject: ${escapeHtml(matched.productModel)} ${matched.brand ? `(${escapeHtml(matched.brand)})` : ''}
          </div>
        ` : ''}
      </section>
        ...
        <div class="p-4 rounded-2xl bg-amber-50/80 border border-amber-200 flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <div class="flex items-center text-amber-400 text-sm">
              ${renderClientReviewStars(matched.rating)}
            </div>
          </div>
          <span class="text-lg font-black text-amber-900">${Number(matched.rating).toFixed(1)} / 5.0</span>
        </div>
      ` : ''}

      <!-- Pros and Cons (Only if manually provided) -->
      ${(matched.pros && matched.pros.length > 0) || (matched.cons && matched.cons.length > 0) ? `
        <section class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ${matched.pros && matched.pros.length > 0 ? `
          ...
          ${matched.cons && matched.cons.length > 0 ? `
      ...
      ${matched.productSlug ? `
        <section class="p-6 rounded-2xl bg-brand-50/60 border border-brand-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="space-y-1 text-center sm:text-left">
            ...
            <div class="text-sm font-extrabold text-slate-900">Looking for full laboratory specifications for ${escapeHtml(matched.productModel || 'this vacuum')}?</div>
          </div>
          <a href="/vacuum/${matched.productSlug}" class="px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs shadow-xs transition shrink-0">
...
function bindReviewDetailPageEvents() {
  document.querySelectorAll('.edit-review-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.preventDefault();
      const articleId = btn.getAttribute('data-edit-review');
...
/**
 * Editorial Review Article Creator & Editor Modal
 * Enables full manual article authoring, editing, draft/publishing, and deletion
 */
function openReviewArticleEditor(article = null) {
  const isEditing = Boolean(article && article.id);
  const existingModal = document.getElementById('review-editor-modal');
  if (existingModal) existingModal.remove();

  const modal = document.createElement('div');
  modal.id = 'review-editor-modal';
  modal.className = 'fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 sm:p-6';

  const defaultTitle = article ? article.title : '';
  const defaultSlug = article ? article.slug : '';
  const defaultAuthor = article ? (article.author || 'Editorial Team') : 'Marcus Thorne';
  const defaultRole = article ? (article.authorRole || 'Lead Testing Specialist') : 'Lead Testing Specialist';
  const defaultCategory = article ? (article.category || 'Cordless Stick') : 'Cordless Stick';
  const defaultReviewType = article ? (article.reviewType || 'Hands-On Lab Test') : 'Hands-On Lab Test';
  const defaultBrand = article ? (article.brand || '') : '';
  const defaultModel = article ? (article.productModel || '') : '';
  const defaultProductSlug = article ? (article.productSlug || '') : '';
  const defaultRating = article && article.rating ? article.rating : '';
  const defaultFeaturedImage = article ? (article.featuredImage || article.coverImage || '') : 'https://images.unsplash.com/photo-1558317374-067fb5f30001?auto=format&fit=crop&w=1200&q=80';
  const defaultExcerpt = article ? (article.excerpt || '') : '';
  const defaultContent = article ? (article.content || (article.sections ? article.sections.map(s => `## ${s.heading}\n\n${s.body}`).join('\n\n') : '')) : '';
  const defaultVerdict = article ? (article.verdict || '') : '';
  const defaultPros = article && Array.isArray(article.pros) ? article.pros.join('\n') : '';
  const defaultCons = article && Array.isArray(article.cons) ? article.cons.join('\n') : '';
  const defaultTags = article && Array.isArray(article.tags) ? article.tags.join(', ') : '';
  const isPublished = article ? article.published === true : true;
  const isFeatured = article ? article.featured === true : false;

  modal.innerHTML = `
    <div class="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-scale-up">
      <!-- Modal Header -->
      <div class="p-6 border-b border-slate-200 flex items-center justify-between bg-slate-50 shrink-0">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-brand-600 text-white flex items-center justify-center font-bold text-lg shadow-xs">
            <i class="fa-solid fa-pen-nib"></i>
          </div>
          <div>
            <h2 class="text-xl font-black text-slate-900">${isEditing ? 'Edit Review Article' : 'Write &amp; Publish Review Article'}</h2>
            <p class="text-xs text-slate-500 font-medium">Create authentic editorial review posts for the /reviews archive.</p>
          </div>
        </div>
        <button type="button" id="close-review-editor-btn" class="w-9 h-9 rounded-xl hover:bg-slate-200 text-slate-500 flex items-center justify-center transition cursor-pointer">
          <i class="fa-solid fa-xmark text-base"></i>
        </button>
      </div>

      <!-- Modal Body (Scrollable) -->
      <form id="review-article-form" class="p-6 sm:p-8 overflow-y-auto space-y-6 flex-1 text-slate-800 text-xs sm:text-sm">
        
        <!-- Article Basics -->
        <div class="space-y-4">
          <h3 class="text-xs font-bold uppercase tracking-wider text-brand-600 flex items-center gap-1.5">
            <i class="fa-solid fa-heading"></i> Article Identity
          </h3>

          <div class="space-y-1.5">
            <label class="block font-bold text-slate-700">Article Headline / Title *</label>
            <input type="text" id="editor-title" required value="${escapeHtml(defaultTitle)}" placeholder="e.g. Dyson V15 Detect Review: Laser Fluffy Head &amp; Suction Test Results" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium" />
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">URL Slug *</label>
              <input type="text" id="editor-slug" required value="${escapeHtml(defaultSlug)}" placeholder="dyson-v15-detect-review" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500 font-mono text-xs" />
              <span class="text-[11px] text-slate-400">Path: /reviews/&lt;slug&gt;</span>
            </div>

            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Review Category *</label>
              <select id="editor-category" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium bg-white">
                <option value="Cordless Stick" ${defaultCategory === 'Cordless Stick' ? 'selected' : ''}>Cordless Stick</option>
                <option value="Robot Vacuums" ${defaultCategory === 'Robot Vacuums' ? 'selected' : ''}>Robot Vacuums</option>
                <option value="Canister Vacuums" ${defaultCategory === 'Canister Vacuums' ? 'selected' : ''}>Canister Vacuums</option>
                <option value="Wet &amp; Dry Vacuums" ${defaultCategory === 'Wet & Dry Vacuums' || defaultCategory === 'Wet & Dry' ? 'selected' : ''}>Wet &amp; Dry Vacuums</option>
                <option value="Upright Vacuums" ${defaultCategory === 'Upright Vacuums' ? 'selected' : ''}>Upright Vacuums</option>
                <option value="Handheld Vacuums" ${defaultCategory === 'Handheld Vacuums' ? 'selected' : ''}>Handheld Vacuums</option>
              </select>
            </div>
          </div>

          <div class="space-y-1.5">
            <label class="block font-bold text-slate-700">Article Excerpt (Archive &amp; SEO Meta Description) *</label>
            <textarea id="editor-excerpt" rows="2" required placeholder="Write a concise 1-2 sentence overview of your testing findings..." class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500">${escapeHtml(defaultExcerpt)}</textarea>
          </div>
        </div>
        ...
        <div class="space-y-4 pt-2 border-t border-slate-200">
          <h3 class="text-xs font-bold uppercase tracking-wider text-brand-600 flex items-center gap-1.5">
            <i class="fa-solid fa-user-pen"></i> Author &amp; Subject Details
          </h3>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Author Name</label>
              <input type="text" id="editor-author" value="${escapeHtml(defaultAuthor)}" placeholder="Editorial Specialist" class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Author Title / Role</label>
              <input type="text" id="editor-author-role" value="${escapeHtml(defaultRole)}" placeholder="Testing Specialist" class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Vacuum Brand</label>
              <input type="text" id="editor-brand" value="${escapeHtml(defaultBrand)}" placeholder="e.g. Dyson" class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Product Model</label>
              <input type="text" id="editor-model" value="${escapeHtml(defaultModel)}" placeholder="e.g. V15 Detect" class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Review Type</label>
              <input type="text" id="editor-review-type" value="${escapeHtml(defaultReviewType)}" placeholder="Hands-On Lab Test" class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4 pt-2 border-t border-slate-200">
          <h3 class="text-xs font-bold uppercase tracking-wider text-brand-600 flex items-center gap-1.5">
            <i class="fa-solid fa-image"></i> Featured Image
          </h3>

          <div class="space-y-1.5">
            <label class="block font-bold text-slate-700">Cover Image URL *</label>
            <input type="url" id="editor-featured-image" required value="${escapeHtml(defaultFeaturedImage)}" placeholder="https://..." class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500 font-mono text-xs" />
          </div>
        </div>

        <!-- Article Content (The verbatim text) -->
        <div class="space-y-4 pt-2 border-t border-slate-200">
          <div class="flex items-center justify-between">
            <h3 class="text-xs font-bold uppercase tracking-wider text-brand-600 flex items-center gap-1.5">
              <i class="fa-solid fa-file-lines"></i> Complete Article Content *
            </h3>
            <span class="text-[11px] text-slate-400 font-semibold">Supports Markdown (# Heading, - List, > Quote)</span>
          </div>

          <div class="space-y-1.5">
            <textarea id="editor-content" rows="12" required placeholder="Type or paste your complete, original review article here. This content is preserved verbatim and rendered with full typographic fidelity..." class="w-full px-4 py-3 rounded-2xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500 font-mono text-xs leading-relaxed">${escapeHtml(defaultContent)}</textarea>
          </div>
        </div>

        <!-- Editorial Additions (Optional manual ratings/verdict) -->
        <div class="space-y-4 pt-2 border-t border-slate-200">
          <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
            <i class="fa-solid fa-award"></i> Editorial Rating &amp; Summary (Optional)
          </h3>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Overall Rating (1.0 - 5.0)</label>
              <input type="number" id="editor-rating" step="0.1" min="1.0" max="5.0" value="${defaultRating}" placeholder="e.g. 4.8" class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
            <div class="space-y-1.5">
              <label class="block font-bold text-slate-700">Tags (comma-separated)</label>
              <input type="text" id="editor-tags" value="${escapeHtml(defaultTags)}" placeholder="cordless, laser, pet hair, hepa" class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          </div>

          <div class="space-y-1.5">
            <label class="block font-bold text-slate-700">Lab Verdict</label>
            <textarea id="editor-verdict" rows="2" placeholder="Summary verdict from your testing..." class="w-full px-4 py-2 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500">${escapeHtml(defaultVerdict)}</textarea>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <label class="block font-bold text-emerald-800">What We Like (One pro per line)</label>
              <textarea id="editor-pros" rows="3" placeholder="Unrivaled suction on deep carpet&#10;Illuminated fluffy brush detects hidden dust" class="w-full px-4 py-2 rounded-xl border border-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xs">${escapeHtml(defaultPros)}</textarea>
            </div>
            <div class="space-y-1.5">
              <label class="block font-bold text-rose-800">What Could Be Better (One con per line)</label>
              <textarea id="editor-cons" rows="3" placeholder="Heavy when using overhead tools&#10;Premium pricing" class="w-full px-4 py-2 rounded-xl border border-rose-300 focus:outline-none focus:ring-2 focus:ring-rose-500 text-xs">${escapeHtml(defaultCons)}</textarea>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex flex-wrap items-center justify-between gap-4">
          <div class="flex items-center gap-6">
            <label class="inline-flex items-center gap-2 cursor-pointer font-bold text-slate-800">
              <input type="checkbox" id="editor-published" ${isPublished ? 'checked' : ''} class="w-4 h-4 rounded text-brand-600 focus:ring-brand-500 border-slate-300" />
              <span>Publish on Website (Appears in /reviews)</span>
            </label>

            <label class="inline-flex items-center gap-2 cursor-pointer font-semibold text-slate-700">
              <input type="checkbox" id="editor-featured" ${isFeatured ? 'checked' : ''} class="w-4 h-4 rounded text-amber-500 focus:ring-amber-400 border-slate-300" />
      ...
      <!-- Modal Footer -->
      <div class="p-6 border-t border-slate-200 bg-slate-50 flex items-center justify-between gap-4 shrink-0">
        <div>
          ${isEditing ? `
            <button type="button" id="btn-delete-review" class="px-4 py-2 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-100 font-bold text-xs border border-rose-200 transition cursor-pointer">
              <i class="fa-solid fa-trash-can mr-1"></i> Delete Article
            </button>
          ` : ''}
        </div>

        <div class="flex items-center gap-3">
          <button type="button" id="btn-cancel-editor" class="px-5 py-2.5 rounded-xl border border-slate-300 text-slate-700 font-bold text-xs hover:bg-slate-200 transition cursor-pointer">
            Cancel
          </button>
          <button type="button" id="btn-save-draft" class="px-5 py-2.5 rounded-xl bg-slate-800 text-slate-200 font-bold text-xs hover:bg-slate-700 transition cursor-pointer">
            Save Draft
          </button>
          <button type="button" id="btn-save-publish" class="px-6 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-black text-xs shadow-md transition cursor-pointer">
            <i class="fa-solid fa-cloud-arrow-up mr-1"></i> Save &amp; Publish
          </button>
        </div>
      </div>
    </div>
  ...
  // Auto slug generation from title if title changes and slug is empty/auto
  ...
  // Close handlers
  const closeBtn = document.getElementById('close-review-editor-btn');
  ...
  async function submitArticle(shouldPublish) {
    ...
    let slug = (document.getElementById('editor-slug').value || '').trim();
    if (!title) {
    ...
    const author = (document.getElementById('editor-author').value || 'Editorial Team').trim();
    const authorRole = (document.getElementById('editor-author-role').value || 'Testing Specialist').trim();
    const brand = (document.getElementById('editor-brand').value || '').trim();
    const productModel = (document.getElementById('editor-model').value || '').trim();
    const reviewType = (document.getElementById('editor-review-type').value || 'Hands-On Lab Test').trim();
    const featuredImage = (document.getElementById('editor-featured-image').value || '').trim();
    const ratingRaw = document.getElementById('editor-rating').value;
    const rating = ratingRaw ? parseFloat(ratingRaw) : null;
    const verdict = (document.getElementById('editor-verdict').value || '').trim();
    const prosRaw = document.getElementById('editor-pros').value || '';
    const pros = prosRaw.split('\n').map(p => p.trim()).filter(Boolean);
    const consRaw = document.getElementById('editor-cons').value || '';
    ...
    const payload = {
      ...
      publishedDate: article && article.publishedDate ? article.publishedDate : new Date().toISOString().split('T')[0]
    };

    try {
      let res;
      if (isEditing) {
        res = await fetch(`/api/review-articles/${article.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      } else {
        res = await fetch('/api/review-articles', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      ...
      if (!res.ok) {
        ...
        alert(errJson.error || 'Failed to save review article.');
        return;
      }

      const savedData = await res.json();
      cachedReviewArticlesList = null; // Invalidate client cache
      closeModal();

      // Navigate to review detail or refresh archive
      if (shouldPublish) {
        navigateTo(`/reviews/${savedData.slug}`);
      } else {
        navigateTo('/reviews');
      }
    } catch (err) {
      console.error('Error saving article:', err);
      alert('Network error saving article. Please try again.');
    }
  }
  ...
  if (deleteBtn) {
    deleteBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await fetch(`/api/review-articles/${article.id}`, { method: 'DELETE' });
        if (res.ok) {
          cachedReviewArticlesList = null;
          closeModal();
          navigateTo('/reviews');
        } else {
  ...
  if (draftBtn) draftBtn.addEventListener('click', () => submitArticle(false));

  const publishBtn = document.getElementById('btn-save-publish');
  if (publishBtn) publishBtn.addEventListener('click', () => submitArticle(true));
}
